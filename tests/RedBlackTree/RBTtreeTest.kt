...
internal class RBTtreeTest {
    val RBTree = RBTtree<Int, Int>()
    var maxBlackHeight = -1

    fun checkStructure(node: RBTnode<Int, Int>? = RBTree.root, blackHeight: Int = 1): Boolean {
        if (node == null)
            return true
        ...
        if (node.col != Colour.black) {

            if (node.left?.col == Colour.red || node.right?.col == Colour.red)
                return false
            else
    ...
    fun rootColorAfterInsert() {
        for (i in 1..100) {
            RBTree.insert(i, i)
            assertEquals(RBTree.root?.col == Colour.black, true)
        }
    }
